...
import {
  ...
  FormControl, InputLabel, Select, MenuItem, TextField, Button, Grid, Paper
} from "@mui/material";
  ...
  return (
    <Box sx={{ maxWidth: 1200, margin: "0 auto", p: 3 }}>
      ...
      <Paper sx={{ p: 2, mb: 3 }}>
        ...
        <Button variant="contained" sx={{ mt: 2 }} onClick={() => setFilters({ ...filters })}>
          Apply Filters
        </Button>
      </Paper>
      ...
      {loading ? (
        ...
        loads.map(load => (<LoadCard key={load._id} load={load} onViewDetails={() => setSelectedLoad(load)} />))
      ) : (<Typography>No loads found.</Typography>)}

      {selectedLoad && <LoadDetailsModal load={selectedLoad} userRole={userRole} onClose={() => setSelectedLoad(null)} onLoadAccepted={handleLoadAccepted} />}

      <Snackbar open={snackbarOpen} autoHideDuration={3000} onClose={handleSnackbarClose}><Alert severity="success">{snackbarMessage}</Alert></Snackbar>

      {activeLoadId && <CarrierLocationTracker loadId={activeLoadId} />}
    </Box>
  );
}
