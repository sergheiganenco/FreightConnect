...
/**
 * Company team management, shared by the carrier and shipper dashboards.
 * The company sub-account model is the same for both (owner + dispatcher/driver);
 * only the role choices and wording differ by marketplace role.
 */
export default function TeamManager() {
  const marketRole = localStorage.getItem('role') || 'carrier';
  const isCarrier = marketRole === 'carrier';

  const roleOptions = isCarrier
    ? [
        { value: 'dispatcher', label: 'Dispatcher — books & manages loads and fleet' },
        { value: 'driver', label: 'Driver — accepts loads, logs HOS, uploads POD' },
      ]
    : [{ value: 'dispatcher', label: 'Coordinator — posts & manages loads' }];

  // A "dispatcher" under a shipper is shown as "Coordinator".
  const roleLabel = (r) => {
    if (r === 'owner') return 'Owner';
    if (r === 'driver') return 'Driver';
    return isCarrier ? 'Dispatcher' : 'Coordinator';
  };

  const description = isCarrier
    ? "Dispatcher and driver logins under your company. They act on your company's behalf, and their actions are tracked to them."
    : 'Coordinator logins under your company. They post and manage loads on your behalf.';
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {error && <Alert severity={error.includes('owner') ? 'info' : 'error'} sx={{ my: 2 }} onClose={() => setError('')}>{error}</Alert>}
          ...
          <TableBody>
            ...
            {members.map((m) => (
              <TableRow key={m._id} sx={{ opacity: m.active ? 1 : 0.55 }}>
                ...
                <TableCell>
                  <Chip size="small" label={roleLabel(m.companyRole)} color={ROLE_COLOR[m.companyRole] || 'default'} />
                </TableCell>
                <TableCell>
                  <Chip
                    size="small"
